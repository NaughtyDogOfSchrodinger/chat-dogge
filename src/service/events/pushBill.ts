...
export const pushChatBill = async ({
  ...
  try {
    ...
    console.log(
      `chat generate success. text len: ${text.length}. token len: ${tokens}`
    )
...
export const pushSplitDataBill = async ({
  ...
  try {
    console.log(
      `splitData generate success. text len: ${text.length}. token len: ${tokenLen}`
    )
    ...
    if (isPay) {
      try {
        ...
        const modelItem = modelList.find(
          (item) => item.model === ChatModelNameEnum.GPT35
        )
...
export const pushGenerateVectorBill = async ({
  ...
  try {
    console.log(
      `vector generate success. text len: ${text.length}. token len: ${tokenLen}`
    )
